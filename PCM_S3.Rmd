...
El modelo de movimiento browniano (Brownian motion model) es un modelo estocástico de paseo aleatorio y de tiempo continuo en el que los cambios de un período de tiempo al siguiente son extracciones aleatorias de una distribución normal con una media de cero y una varianza σ2 (Felsenstein 1973). Dadas estas condiciones, la varianza esperada bajo el modelo browniano aumenta linealmente a lo largo del tiempo con una tasa instantánea que es equivalente a la varianza de la distribución normal a partir de la cual se dibujan los cambios evolutivos.
...
En este caso, extraeremos nuestros cambios evolutivos a partir de una distribución normal; sin embargo, probablemente valga la pena señalar que, independientemente de la distribución, la evolución procederá según el movimiento browniano a medida que la amplitud de nuestros pasos de tiempo disminuye hacia cero, siempre y cuando se cumplan otras suposiciones razonables.
...
Quizás hayas notado que en lugar de simular la evolución browniana aditiva paso a paso, comenzamos simulando todos los pasos, luego procedimos a calcular el estado del proceso a lo largo del tiempo calculando la suma acumulada desde el tiempo 0 hasta t para todos los valores posibles de t. Esto es posible porque una propiedad del modelo es que la distribución de cambios bajo el movimiento browniano es invariante y no depende del estado anterior en el proceso de cambio.
...
La magnitud del parametro sigma2, la tasa instantanea de cambio en el proceso Browniano es cla ve en el proceso browniano.
...
object<-simBMphylo(n=6,t=100,sig2=0.01,fsize=0.8,cex.axis=0.6,cex.lab=0.8,las=1)
```

De nuevo podemos apreciar como la cantidad de cambio acumulado aumenta con el tiempo.
La diferencia aquí es que los linajes solo pueden comenzar a diferenciarse una vez han divergido. Y la cantidad de cambio acumulado entre la especies es directamente proporcional al tiempo que hace que divergieron y inversamente proporcional al tiempo que comparte coancestría desde la raiz.

PROPIEDADES DEL MODELO BROWNIANO

El modelo browniano es un modelo de movimiento estocástico y sin una dirección definida. En cada lapso de tiempo podría tener un movimiento en una dirección y en la siguiente en la otra dirección volviendo al sitio inicial. En promedio no hay una tendencia de movimiento hacia arriba o hacia abajo.
En este modelo la variación se acumula en función del tiempo. Hemos podido ver en la simulaciones como esa variación se acumula con el tiempo. La tasa de variación es equivalente la tasa de cambio instantaneo en el modelo browniano que hemnos denominado sigma2.
...
COMO AJUSTAR EL MODELO BRWONIANO A UN SET DE DATOS

Necesitamos ajustar dos parámetros. Por una lado el parámetro de varianza instantanea del modelo brwoniano, que hemos llamada sigma2. Y en segundo lugar necesitamos también estimar el valor de nuestro fenotipo bajo estudio en la raíz de la filogenia.

Para estimar estos dos parámetros a partir de nuestra filogenia y los valores del un fenotipo para las especies de esa filogenia y un criterio para decidir cuales son los valores σ2 y x0 que mejor se ajustan a nuestros datos (filogenia y valores del fenotipo de las especies).
...
Esto quiere decir que a partir de un genoma original de 1.98 Mb esperamos que por cada unidad de tiempo este valor cambie con una varianza de 25Mb2. Además de estas estimas tenemos el valor de AIC y AICc que nos permitirá comparar este modelos con otros modelos alternativos.
...
La función MLE usa una optimización numérica y nos da el dato de cuantas iteraciones independiente de optimización ha hecho en el análisis de MLE y cuantas veces ha convergido en la mejor solución encontrada. En este caso fueron el 100% de la veces.
...
Si nos fijamos los datos brutos muestras una distribución muy inclinada a la izquierda. Si los tansformamos con una función logaritmica, muestran una distribución mucho más parecida a la normal y esto ayudará al ajuste del modelo.
...
La señal filogenética de un fenotipo se observa cuando las especies más cercanas filogenéticamente se parecen más en su fenotipo de los que esperaríamos al azar. Esto está muy relacionado con el modelo browniano, ya que un fenotipo con señal filogenética debe de ajustarse bastante bien a un modelo browniano por lo que hemos explicado anteriormente.
...
En ambos casos odemos rechazar la hipotesis nula de lambda = 0.
...
Podemos verlo ahora con el criterio AICc.
...
Usa los datos de estimas de porcentage de GC en bacterias ("GC_Content_Percent"). Ajusta los modelos BM and OU y comparalos. Calcula la señal filogenética de este fenotipo. 
